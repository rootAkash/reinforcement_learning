# here gae and 2 actor nets are used
#later will implement a simple advantage function and a single actor neural net 
...
policy.summary()
old_policy = tf.keras.Model(inputs= state_inputs, outputs= action_outputs, name='old_actor_model')
...
def update_old_actor():
	new_policy_weights= policy.get_weights()
	old_policy_weights=old_policy.get_weights()
	#for i in range(len(new_policy_weights)):
	#	old_policy_weights[i]=new_policy_weights[i]
	old_policy.set_weights(new_policy_weights)
def onehot(a,s):
	i = np.zeros(a_dim)
	i[a]=1
	return i
#C:\Users\Dell\Desktop\holidASY\my_ppo\ppo_tf.keras	
def save_weights():
...
for episode in range(1,episodes):
	...
	while not done:
		...
		pred_action = old_policy.predict(np.array([s]))# prob distribution
		...
		#if  episode<50:
		#	action = random_action # some exploration		
		action_one_hot=onehot(action,a_dim)# acton matrix
		s_, reward, done, info = env.step(action-1)
		reward= s_[3] + s_[1]
	...
	print("episode: "+ str(episode)+" steps taken : " + str(len(obs)))
