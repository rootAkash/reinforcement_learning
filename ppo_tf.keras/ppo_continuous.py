...
def proximal_policy_optimization_loss(advantage, action):
	...
	def loss(y_true, y_pred):
		...
		#old_mu = tf.keras.backend.dot(y_true,mu_mask)
		#old_sigma = tf.keras.backend.dot(y_true,sig_mask)
...
policy.compile(loss=proximal_policy_optimization_loss(advantage=advantage,action=action), optimizer=tf.keras.optimizers.Adam(lr=0.001))
policy.summary()
...
for episode in range(1,episodes):
	...
	for step in range(steps):
		...
		memory.store(s.ravel(),action,s_.ravel(),reward,done,out[0])# s, a, s_, r, done,musig
		rews+=reward
		if done:
			s_=env.reset()
	...
	policy.fit(x=(obs,advantage,Action),y=(Old_Prediction_musig),batch_size=64,shuffle=True, epochs=10, verbose=False)
