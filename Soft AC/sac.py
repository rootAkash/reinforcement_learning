...
def save_actor_weights():
	actorpath=r"C:\\Users\\Dell\\Desktop\\holidASY\\Soft AC\\SACpend_actor.h5"
...
ctr = 0
render =False
train_iter = 1000  # 1000, batch of 8/16 works best for now for squeezing everything from data 
#load_actor_weights()
s = env.reset()
for ep in range(episodes):
	...
	for step in range(steps):
		...
		s_,r,done,_=env.step(np.array([Action*a_bound]))# since the action that the networks are taking into account is [-1,1]
		# but the env might have different mapping so we take step by multipying action with a_bound makes it [-a_bound,a_bound]
